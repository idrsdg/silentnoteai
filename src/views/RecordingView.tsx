...
export default function RecordingView({ licenseStatus, onSessionSaved, onGetLicense }: Props = {}) {
  ...
  return (
    <div style={{ padding: '28px 32px', height: '100%', display: 'flex', flexDirection: 'column', gap: '20px', overflowY: 'auto' }}>
      ...
      {isExpired && (
        <div style={{ padding: '24px', borderRadius: '14px', textAlign: 'center', background: '#120f0b', border: '1px solid #3a2a00' }}>
          <div style={{ fontSize: '32px', marginBottom: '10px' }}>🔒</div>
          <div style={{ fontSize: '16px', fontWeight: 700, marginBottom: '6px' }}>{t.record.trialEnded}</div>
          <button
            onClick={() => onGetLicense?.()}
            style={{ padding: '9px 22px', borderRadius: '9px', border: 'none', background: '#f59e0b', color: '#fff', fontSize: '13px', fontWeight: 600, cursor: 'pointer' }}
          >
            {t.record.getLicense}
          </button>
        </div>
      ...
      {!isExpired && (
        <div style={{
          background: '#120f0b', borderRadius: '14px', padding: '28px',
          border: `1px solid ${state === 'recording' ? '#f59e0b' : '#222'}`,
          transition: 'border-color 0.3s',
        }}>
          {state === 'recording' && (
            <div style={{ display: 'flex', gap: '8px', marginBottom: '16px', justifyContent: 'center' }}>
              <span style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '6px', background: 'rgba(245,158,11,0.08)', border: '1px solid #f59e0b', color: '#fde68a' }}>{t.record.mic}</span>
              {hasSysAudio
                ...
                : <span style={{ fontSize: '11px', padding: '3px 8px', borderRadius: '6px', background: '#1c1814', border: '1px solid #2a2218', color: '#52525b' }}>{t.record.noSysAudio}</span>
              }
            </div>
          )}
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '3px', height: '44px', marginBottom: '20px' }}>
            {bars.map((h, i) => (
              <div key={i} style={{ width: '3px', height: `${h}px`, borderRadius: '2px', background: state === 'recording' ? '#f59e0b' : '#2a2218' }} />
            ))}
          </div>

          {/* Timer */}
          <div style={{ textAlign: 'center', marginBottom: '22px' }}>
            <div style={{ fontSize: '38px', fontWeight: 700, fontVariantNumeric: 'tabular-nums', color: state === 'recording' ? '#fde68a' : '#333', letterSpacing: '-1px' }}>
              {fmt(elapsed)}
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'center', gap: '10px', flexWrap: 'wrap' }}>
            {state === 'idle' && <Btn color="#f59e0b" onClick={handleStartClick}>{t.record.start}</Btn>}
      ...
      {state === 'transcribed' && !error && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '14px', flex: 1, minHeight: 0 }}>
          ...
          <div style={{ background: '#120f0b', borderRadius: '14px', padding: '20px', border: '1px solid #2a1a0a' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
              {MODES.map(m => (
                <button
                  ...
                  style={{
                    padding: '14px 10px', borderRadius: '10px', border: '1px solid #2a2218',
                    background: '#0d0b08', color: '#e5e5e5', cursor: 'pointer',
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '6px',
                    transition: 'border-color 0.15s, background 0.15s',
                  }}
                  onMouseEnter={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = '#f59e0b'; (e.currentTarget as HTMLButtonElement).style.background = '#111'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLButtonElement).style.borderColor = '#2a2218'; (e.currentTarget as HTMLButtonElement).style.background = '#0d0b08'; }}
                >
      ...
      {transcript && (state === 'done' || state === 'analyzing') && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', flex: 1, minHeight: 0 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            {/* Audio player */}
            {audioSrc && (
              <div style={{ background: '#120f0b', borderRadius: '10px', padding: '12px 14px', border: '1px solid #222' }}>
                ...
                <audio
                  ...
                  src={audioSrc}
                  controls
                  style={{ width: '100%', height: '32px', accentColor: '#f59e0b' }}
                />
              </div>
            )}
            ...
            {utterances.length > 0 && audioRef.current && (
              <div style={{ background: '#120f0b', borderRadius: '10px', padding: '12px 14px', border: '1px solid #222', maxHeight: '160px', overflowY: 'auto' }}>
                <div style={{ fontSize: '11px', color: '#555', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '8px' }}>
                ...
                {utterances.map((u, i) => (
                  <div
                    ...
                    style={{ fontSize: '12px', color: '#aaa', padding: '4px 0', cursor: 'pointer', lineHeight: '1.5', borderBottom: '1px solid #2a2218' }}
                    onMouseEnter={e => (e.currentTarget.style.color = '#fde68a')}
                    onMouseLeave={e => (e.currentTarget.style.color = '#aaa')}
                  >
                    <span style={{ color: '#f59e0b', fontWeight: 600 }}>{u.speaker}:</span> {u.text}
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            {actions.length > 0 && (
              <Card title={t.record.results.actions}>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {actions.map((a, i) => (
                    <div key={i} style={{ background: '#0d0b08', borderRadius: '8px', padding: '10px 12px', border: '1px solid #1e1e1e', fontSize: '12px' }}>
                      <div style={{ color: '#e5e5e5', marginBottom: '4px', fontWeight: 500 }}>{a.task}</div>
                      <span style={{ color: '#fb923c' }}>{a.owner}</span>
            ...
            {state === 'done' && (
              <Card title={exportTitle}>
                ...
                <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                  {(['txt', 'md', 'pdf', 'docx'] as ExportFormat[]).map(format => (
                    <button
                      ...
                      style={{
                        padding: '8px 12px', borderRadius: '8px', border: '1px solid #2a2218',
                        background: '#0d0b08', color: '#ddd', fontSize: '12px', cursor: exportingFormat ? 'not-allowed' : 'pointer',
                ...
                {exportPath && (
                  <div style={{ fontSize: '11px', color: '#f59e0b', marginTop: '12px', lineHeight: '1.6', wordBreak: 'break-all' }}>
            ...
            {state === 'done' && (
              <div style={{ display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                <Btn color="#f59e0b" onClick={() => void finalizeSession()} disabled={saveBusy || isSessionFinalized}>
            ...
            {state === 'done' && isSessionFinalized && (
              <div style={{ fontSize: '12px', color: '#fde68a', padding: '8px 12px', background: 'rgba(245,158,11,0.08)', borderRadius: '8px', border: '1px solid rgba(245,158,11,0.2)' }}>
            ...
            <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: consentChecked ? '#f59e0b' : '#444', color: '#fff',
...
function Card({ title, children }: { title: string; children: React.ReactNode }) {
  return (
    <div style={{ background: '#120f0b', borderRadius: '12px', padding: '18px 20px', border: '1px solid #222' }}>
